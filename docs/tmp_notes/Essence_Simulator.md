很多所谓“仿真类型”并不是新的底层求解问题，而是对几个核心仿真的**包装、复用、循环调用或后处理**。

如果从“仿真器底层到底在解什么方程”的角度看，最本质的仿真其实没有那么多，核心可以压缩成几类。
...
下面这些常见仿真，本质上不是新的底层仿真，而是对 DC、Transient、AC 的重复调用、参数循环或后处理。
...
$$
\text{Noise, TF, PZ, STB, S-parameter, PAC, PNoise}
$$
...
$$
\text{Sweep, Corner, Monte Carlo, Worst-case, Measure, Optimization}
$$
