...
等这一轮 `unknown -> F -> Q -> merge` 主线真正稳定以后，最值得单独补的一篇，就是：
...
- 初值处理
- NQS 相关的工程化处理
- 为了 Newton 收敛而加入的各种 guard 逻辑

这一部分之所以适合作为下一轮展望，而不是现在就并入主线，是因为它更像：

```text
物理模型之上的数值实现层
```

先把 `B4` 的 `F/Q/dFdx/dQdx` 主干读顺，再回头单独研究这些 guard，层次会更清楚。
