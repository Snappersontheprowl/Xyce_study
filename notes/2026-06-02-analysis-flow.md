...
这一点是第五阶段最值得先搞清楚的地方之一。

先看：

- [N_ANP_DCSweep.C](../vendor/Xyce-7.10.0/src/AnalysisPKG/N_ANP_DCSweep.C) 第 990 行附近
  `extractOPData(...)`

这里做的事情非常轻：

- 创建一个 `OptionBlock("OP", ...)`
- 如果 `.OP` 行还有多余字段，发 warning
- 把这个 option block 交给 `circuit_block.addOptions(...)`

也就是说，`.OP` 在 parser 这里更像：

```text
一条“请求做 operating point”的声明
```

而不是：

```text
立刻构造一个单独的 analysis class
```
...
- 如果 `dotOpSpecified_` 为真
- 并且没有别的主分析，或者只有像 `.STEP` / `.SAMPLING` 这样的外层包装分析
- 那么就创建一个 `DCSweep` 作为 primary analysis
